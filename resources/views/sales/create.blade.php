                        ...
                        <div class="flex">
                            <div class="mx-auto sm:px-6 lg:px-6 p-4 w-1/2">
                                <fieldset>
                                    ...
                                    <div class="acciones">
                                        <a href="{{ asset('storage/' . $pullApart->agreement) }}" target="_blank"
                                           class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150 h-10 bg-blue-500">Ver
                                            Cotización</a>
                                        @if($pullApart->is_sale === 1)
                                            <form action="{{ route('sales.destroy', $pullApart->id) }}" method="POST">
                                                ...
                                                @csrf
                                                <button type="submit"
                                                        class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150 h-10 bg-blue-500">
                                                    Anular Venta
                                                </button>
                                            </form>
                                        @endif
                                    ...
                                    <div class="tabla">
                                        <table cellspadding="0" celspacind="0" border="0">
                                            ...
                                            @if($pullApart->visit->closets->count() > 0)
                                                ...
                                                @endforeach
                                            @endif
                                            @if(!is_null($pullApart->discount))
                                                <tr>
                                                    <td>Descuento</td>
                                                    <td></td>
                                                    <td>
                                                        @if($pullApart->discount_type ===  2)
                                                            {{ number_format($pullApart->discount,2) }} %
                                                        @else
                                                            @if($pullApart->visit->project->currency === 'PEN')
                                                                S/. {{ number_format($pullApart->discount,2) }}
                                                            @else
                                                                US$. {{ number_format($pullApart->discount,2) }}
                                                            @endif
                                                        @endif
                                                    </td>
                                                </tr>
